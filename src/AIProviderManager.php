...
class AIProviderManager extends Manager
{
    ...
    {
        return new Anthropic(...config('neuron.provider.anthropic'));
    }

    public function createOpenaiDriver(): AIProviderInterface
    {
        return new OpenAI(...config('neuron.provider.openai'));
    }

    public function createOpenaiResponsesDriver(): AIProviderInterface
    {
        return new OpenAIResponses(...config('neuron.provider.openai-responses'));
    }

    public function createGeminiDriver(): AIProviderInterface
    {
        return new Gemini(...config('neuron.provider.gemini'));
    }

    public function createOllamaDriver(): AIProviderInterface
    {
        return new Ollama(...config('neuron.provider.ollama'));
    }

    public function createMistralDriver(): AIProviderInterface
    {
        return new Mistral(...config('neuron.provider.mistral'));
    }

    public function createDeepseekDriver(): AIProviderInterface
    {
        return new Deepseek(...config('neuron.provider.deepseek'));
    }

    public function createHuggingfaceDriver(): AIProviderInterface
    {
        return new HuggingFace(...config('neuron.provider.huggingface'));
    }
}
